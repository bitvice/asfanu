...
import { FamilyCampaignsTab } from '@/components/campaigns/FamilyCampaignsTab';
import { calculateCurrentAge, parseBirthDate } from '@/lib/children/age';
import { formatVoucherName } from '@/lib/utils';
import { ArrowLeft, Edit, User, Phone, Mail, MapPin, Baby, ShieldCheck, Megaphone } from 'lucide-react';
...
export default function RegistrationDetailPage() {
  ...
  const familyName = formatVoucherName(data.parent_last_name, data.parent_first_name);
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-slate-900 dark:text-slate-100 flex items-center gap-3 flex-wrap">
            <span>{data.parent_last_name} {data.parent_first_name}</span>
            {data.family_number && (
              <Badge variant="outline" className="text-xs sm:text-sm font-mono font-bold bg-indigo-50/50 dark:bg-indigo-950/40 border-indigo-200 dark:border-indigo-800 text-indigo-700 dark:text-indigo-300 px-3 py-1">
                Familia #{String(data.family_number).padStart(3, '0')}
              </Badge>
            )}
          </h1>
        </div>
      ...
      <Tabs defaultValue="details">
        ...
        <TabsContent value="campaigns">
          <Card>
            <CardHeader className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <CardTitle className="text-sm font-bold flex items-center gap-2 text-indigo-600 dark:text-indigo-400">
                <Megaphone className="w-4 h-4" /> Campanii de Reducere
              </CardTitle>
            </CardHeader>
